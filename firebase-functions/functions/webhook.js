...
async function handlePaymentAuthorized(payment) {
  try {
    ...
    await admin.firestore().collection('subscriptionOrders').doc(orderId).update({
      status: 'authorized',
...
async function handlePaymentCaptured(payment) {
  try {
    ...
    // Update order status
    const orderRef = admin.firestore().collection('subscriptionOrders').doc(orderId);
    ...
    batch.update(priestRef, {
      ...
      'subscription.remainingLeads': planDetails.customerRequests === 'Unlimited' ? 999 : planDetails.customerRequests,
    ...
    const notificationRef = admin.firestore().collection('priestNotifications').doc();
...
async function handlePaymentFailed(payment) {
  try {
    ...
    // Create notification for the priest
    await admin.firestore().collection('priestNotifications').add({
      priestId: priestId,
...
async function handleSubscriptionCharged(subscription) {
  try {
    ...
    const orderId = admin.firestore().collection('subscriptionOrders').doc().id;
...
exports.checkExpiringSubscriptions = functions.pubsub.schedule('0 0 * * *')
  ...
  .onRun(async (context) => {
    try {
      ...
      const expiringSubscriptions = await admin.firestore().collection('priests')
...
exports.processSubscriptionRenewals = functions.pubsub.schedule('0 1 * * *')
  ...
  .onRun(async (context) => {
    try {
      ...
      const subscriptionsToRenew = await admin.firestore().collection('priests')
        .where('subscription.expiresAt', '>=', today)
...
async function createRazorpayRenewalPayment(priestId, priestData) {
  try {
    ...
    await admin.firestore().collection('subscriptionOrders').doc(orderId).set({
      priestId: priestId,
...
exports.resetMonthlyLeads = functions.pubsub.schedule('5 0 1 * *')
  ...
  .onRun(async (context) => {
    try {
      ...
      const activePriests = await admin.firestore().collection('priests')
        .where('active', '==', true)
      ...
      for (const doc of activePriests.docs) {
        ...
        const priestRef = admin.firestore().collection('priests').doc(doc.id);
        ...
        currentBatch.set(notificationRef, {
          ...
          message: `Your monthly lead count has been reset to ${planDetails.customerRequests === 'Unlimited' ? 'unlimited' : planDetails.customerRequests}. You can now receive ${planDetails.customerRequests === 'Unlimited' ? 'unlimited' : `up to ${planDetails.customerRequests}`} new customer requests this month.`,
          type: 'leads',
          read: false,
          createdAt: admin.firestore.FieldValue.serverTimestamp()
        });
  ...
  .onRun(async (context) => {
    try {
      ...
      for (const doc of activePriests.docs) {
        ...
        const invoiceRef = admin.firestore().collection('priestInvoices').doc();
        await invoiceRef.set({
          priestId: doc.id,
          priestName: priest.fullName || priest.firstName + ' ' + priest.lastName || priest.name,
        ...
        await admin.firestore().collection('priests').doc(doc.id).update({
...
exports.onBookingStatusChange = functions.firestore
  ...
  .onWrite(async (change, context) => {
    try {
      ...
      const bookingTypeForFee = determineBookingTypeForFee(afterData, bookingType);
      
      // Increment the priest's successful bookings counter
      const successfulBookingsPath = `subscription.successfulBookings.${bookingTypeForFee}`;
      
      await admin.firestore().collection('priests').doc(priestId).update({
...
exports.onPriestRequestCreated = functions.firestore
  ...
  .onCreate(async (snapshot, context) => {
    try {
      ...
      // Get the priest doc to check if it's a premium subscription
      const priestDoc = await admin.firestore().collection('priests').doc(priestId).get();
      if (!priestDoc.exists) {
        console.error('Priest not found:', priestId);
        return null;
      }
      ...
      // Don't decrement for premium priests since they have unlimited leads
      if (subscription.plan !== 'premium') {
        ...
        console.log(`Skipped decrementing leads for premium priest ${priestId}`);
      }
